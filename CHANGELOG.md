...
- **Windows installer now covers the full setup, not just Sunshine.** Added two steps so a fresh machine reaches a complete state:
  - **Steam auto-install** via winget (`Valve.Steam`), skipped if Steam is already present (checks both the default path and the registry `InstallPath`). The resolved path feeds `apps.json` so it's never hardcoded to a location that doesn't exist. CLI: on by default, `-SkipSteam` to opt out. GUI: "Install Steam if not already present" checkbox.
  - **Virtual display driver** (opt-in) for truly headless hosts with no monitor attached — the Windows analogue of the Linux synthetic-EDID work. Downloads + installs the community Virtual-Display-Driver (IDD), trusting its catalog cert so `pnputil` can install it unattended. CLI: `-InstallVirtualDisplay`. GUI: "Install virtual display driver (for headless / no monitor)" checkbox. Default off, since machines with a real monitor or HDMI dummy plug don't need it. Marked experimental.
  - Sunshine service is now explicitly set to `Automatic` startup so it survives reboots (previously relied on Sunshine's own installer doing this).
